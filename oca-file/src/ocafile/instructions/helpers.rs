            ...
            Rule::key_value => {
                ...
                if let Some(nested_item) = item.clone().into_inner().next() {
                    match nested_item.as_rule() {
                        Rule::string => {
                            // println!("string: {:?}", nested_item.clone().into_inner().last());
                            value = NestedValue::Value(nested_item.clone().into_inner().last().unwrap().as_span().as_str().to_string());
                            //value = next().unwrap().as_str().to_string();
                        }
                        _ => {
                            value = NestedValue::Value(item.as_str().to_string());
                        }
                    }
                    // panic!("Invalid attribute value in {:?}", item.as_rule());
                }

                // value = item.clone().into_inner().next().unwrap().into_span().as_str().to_string();
                // println!("value: {}", value);
            },
